    	...
    	if ( baseA < 2 || baseB < 2 || precisionB < 1) {
            ...
            for (int i = 1; i <= precisionB; i++) {
                ...
                for (int k = 0; k < digits.length; k++) {
                    ...
                    x = (digits[j] * baseB) + carry; //multiply the ith digit by baseB and add the carry
                    digits[j] = x % baseA; //new ith digit is x % baseA
                    carry = x / baseA; //carry = x / baseA
                }
                output[i - 1] = carry; //output[i] = carry
            }
